  ...
  return (
    <form id="mainform" onSubmit={handleSubmit(onSubmit)}>
      ...
      {errors.id && errors.id.type === "required" && (
        <p class="alert">아이디가 입력되지 않았습니다.</p>
      )}
      {errors.id && errors.id.type === "maxLength" && (
        <p class="alert">아이디는 8글자 이내로 입력해주세요.</p>
      )}
      {errors.id && errors.id.type === "pattern" && (
        <p class="alert">아이디는 이내로 입력해주세요.</p>
      )}
      ...
      {errors.name && errors.name.type === "required" && (
        <p class="alert">이름이 입력되지 않았습니다.</p>
      )}
      {errors.name && errors.name.type === "maxLength" && (
        <p class="alert">이름은 10글자 이내로 입력해주세요.</p>
      )}
      ...
      {errors.password && errors.password.type === "required" && (
        <p class="alert">비밀번호가 입력되지 않았습니다.</p>
      )}
      {errors.password && errors.password.type === "minLength" && (
        <p class="alert">비밀번호는 6글자 이상 입력해주세요.</p>
      )}
      {errors.password && errors.password.type === "maxLength" && (
        <p class="alert">비밀번호는 15글자 이내로 입력해주세요.</p>
      )}
